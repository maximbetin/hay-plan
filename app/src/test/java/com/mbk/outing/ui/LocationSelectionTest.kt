...
class LocationSelectionTest {
    ...
    private fun forecast(location: OutingLocation, day: LocalDate = date) = LocationForecast(location,
    ...
    @Test fun `initial state is all towns in Beach mode`() {
        val state = OutingUiState(forecasts = listOf(forecast(gijon), forecast(oviedo)))
    ...
    @Test fun `opening inland town does not change the chosen activity or date`() {
        val state = OutingUiState(forecasts = listOf(forecast(oviedo)), selectedDate = date).openLocation("oviedo")
    ...
    @Test fun `activity change returns to all towns and preserves selected date`() {
        val state = OutingUiState(forecasts = listOf(forecast(oviedo)), selectedDate = date)
    ...
    @Test fun `navigation preserves source timestamps and refresh failure flags`() {
        ...
        val state = OutingUiState(forecasts = listOf(data)).openLocation("oviedo")
    ...
    @Test fun `dates cover all towns and opening one cannot silently change the date`() {
        val state = OutingUiState(forecasts = listOf(forecast(gijon), forecast(oviedo, date.plusDays(1))),
